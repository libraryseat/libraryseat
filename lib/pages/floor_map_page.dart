...
class _FloorMapPageState extends State<FloorMapPage> {
  ...
  Future<void> _silentRefresh() async {
    ...
    try {
      ...
      if (currentSeats == null || _hasSeatChanges(currentSeats, seats)) {
        if (mounted) {
          setState(() {
  ...
  bool _hasSeatChanges(List<SeatResponse> oldSeats, List<SeatResponse> newSeats) {
    ...
    for (int i = 0; i < oldSeats.length; i++) {
      final old = oldSeats[i];
      final newSeat = newSeats.firstWhere(
        (s) => s.seatId == old.seatId,
        orElse: () => old,
      );
  ...
  Widget build(BuildContext context) {
    ...
    final availableHeight = screenSize.height - (isMobile ? 40 : 60);
    ...
    if (isMobile) {
      // 手机：使用可用宽度，高度按比例
      mapWidth = availableWidth - 40; // 留出边距
      mapHeight = mapWidth / aspectRatio;
      // 如果高度超出，则按高度计算
      if (mapHeight > availableHeight) {
        mapHeight = availableHeight - 40;
  ...
  Widget _buildSidebar({bool isMobile = false}) {
    ...
    final sidebarWidth = isMobile ? 60.0 : 80.0;
    final buttonSize = isMobile ? 50.0 : 60.0;
    return Container(
      width: sidebarWidth,
      color: Colors.transparent,
      child: Center(
              ...
              GestureDetector(
                ...
                child: Column(
                  children: [
                    ...
                    if (i == _selectedFloorIndex) const CircleAvatar(backgroundColor: Colors.black54, radius: 3),
                  ],
                ),
              ),
              if (i > 0) const SizedBox(height: 10), // 楼层之间的间距
            ],
          ],
        ),
      ),
